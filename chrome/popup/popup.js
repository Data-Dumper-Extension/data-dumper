...
async function consumePendingSelectors() {
  const result = await chrome.storage.local.get(["lastSelectors", "ddPickingNext"]);
  if (!result.lastSelectors?.length) return false;

  const newSels = result.lastSelectors;
  await chrome.storage.local.remove(["lastSelectors", "ddPickingNext"]);
...
btnSelect.addEventListener("click", async () => {
  await chrome.storage.local.set({ ddPickingNext: false });
...
btnPickNext.addEventListener("click", async () => {
  await chrome.storage.local.set({ ddPickingNext: true });
